...
public class PrintPic {
    ...
    public byte[] printDraw() {
        ...
        // Print the instructions for the raster bitmap
        imgbuf[0] = 29;// Hexadecimal 0x1D
        imgbuf[1] = 118;// Hexadecimal 0x76
        imgbuf[2] = 48;// 30
        imgbuf[3] = 0;// Bitmap mode 0,1,2,3
        // Indicates the number of bytes in the horizontal direction（xL+xH × 256）
        imgbuf[4] = (byte) (this.width / 8);
        imgbuf[5] = 0;
        // Indicates the number of vertical bitots（ yL+ yH × 256）
        ...
        for (int i = 0; i < getLength(); i++) {// The height of the loop bitmap
            for (int k = 0; k < this.width / 8; k++) {// The width of the loop bitmap
                ...
                int p0;
                if (c0 == -1)// Judge the color is not white
                    p0 = 0;// 0, Do not print the point
                else {
                    p0 = 1;// 1, Print the point
                }
